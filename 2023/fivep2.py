...
with open(sys.argv[1]) as fh:
    ...
    last_seeds = []
    for midx in range(0, len(new_seeds)):
        m = new_seeds[midx]
        if m != minseed:
            continue
        start, count = offset_seeds[midx]
        print("actually testing: ", start, count, "with minseed:", minseed, "which is index:", midx)
            ...
            for section in sections:
                ...
                for source_idx in range(0, len(mappings[section]["source"])):
                    ...
                    if seed >= source and seed < source_end:
                        ...
                        seed = nseed
                        break
            last_seeds.append(seed)
    print(new_seeds)
    print(min(new_seeds))
    print(last_seeds)
    print(min(last_seeds))
